...
const Products = () => {
    ...
    const cartIsLoading = useSelector((state) => state.cart.isLoading); // Renamed to avoid conflict
    ...
    const handleAddToCart = (product) => {
        ...
        if (isAuthenticated && !isLocalCart) {
            ...
            try {
                ...
                // Only force update if not already in local cart mode
                if (!isLocalCart) {
    ...
    const renderCartControls = (product) => {
        ...
        if (isAuthenticated && !isLocalCart) {
            cartItem = carts.find(item => item.plant._id === product._id);
        } else {
            cartItem = localCarts.find(item => item.product._id === product._id);
        }
        
        if (cartItem) {
            // Product is in cart, show quantity controls
            const quantity = cartItem.quantity;
            const itemId = isAuthenticated && !isLocalCart ? cartItem._id : cartItem.product._id;
