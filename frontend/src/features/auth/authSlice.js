...
import { handleAuthResponse, safelyClearTokens } from '../../utils/authUtils';
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(userLoginAsync.pending, (state) => {
                ...
                // Reset state first to clear any previous state
                resetToDefaultAuthStore(state);

                // Handle special code cases (verification, 2FA)
                if(action.payload && action.payload.code) {
                    if (action.payload.code === 'VerifyUser') {
                        state.isUserVerificationNeeded = true;
                        state.email = action.meta.arg.email;
                        message.warning(action.payload.message || "You need to verify your account first.");
                        return;
                        ...
                        message.warning(action.payload.message || "Two-factor authentication required.");
                        return;
                    }
                    message.error(action.payload.message || "Authentication issue detected.");
                    return;
                }
                ...
                // Safely handle token storage with comprehensive validation
                try {
                    // Attempt to handle token with multiple possible structures
                    const success = handleAuthResponse(action.payload);
                    
                    if (!success) {
                        resetToDefaultAuthStore(state);
                        message.error("Login failed: Unable to process authentication data");
                        return;
                    }
                    
                    message.success(action.payload.message || "Login successful");
                } catch (error) {
            ...
            .addCase(userLogoutAsync.pending, (state) => {
                ...
                // Use the new auth utils to safely clear tokens
                safelyClearTokens();

                message.success(action.payload.message || 'Logout successful');
            ...
            .addCase(validateTwoFactorAuthAsync.pending, (state) => {
                ...
                // Use the new auth utils to safely handle the token
                const success = handleAuthResponse(action.payload);
                
                // If token handling failed, reset the auth state
                if (!success) {
                    resetToDefaultAuthStore(state);
