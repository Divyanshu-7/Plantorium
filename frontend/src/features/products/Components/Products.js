...
const Products = () => {
    // Ensure the container has enough height to prevent footer overlap
    ...
    const [, forceUpdate] = useState(0); // State just for forcing re-renders
    ...
    // Force component re-rendering when local carts change
    useEffect(() => {
        // This will re-render the component when localCarts changes
    }, [localCarts, carts]);
    ...
    const handleAddToCart = (e, product) => {
        ...
        try {
            ...
            console.log('Local Cart Item:', localCartItem);
            
            dispatch(addToLocalCartAction(localCartItem));
    ...
    const renderCartControls = (product) => {
        ...
        if (isAuthenticated && !isLocalCart) {
            ...
            // For local cart, check both possible structures
            cartItem = localCarts.find(item => {
                // Try to match by product._id 
                if (item.product && item.product._id === product._id) {
                    return true;
                }
                // Try to match by plant._id
    ...
    return (
        <div className="container-fluid product-container products-container">
            ...
            <div className="product-content px-2">
                {
                    products && products.length > 0 && products.map((elem) => (
                        <div key={elem._id} className="px-1 product-row">
