...
const Products = () => {
    ...
    const handleRemoveFromCart = (itemId) => {
        if (isAuthenticated && !isLocalCart) {
            ...
            // Pass itemId directly, not as an object
    ...
    // Render product rating with readonly property
    const renderProductRating = (product) => {
        return (
            <div onClick={(e) => e.stopPropagation()}>
                ...
                />
                <small style={{ position: "relative", top: "4px" }}>{product.noOfRatings}</small>
            </div>
        );
    };
